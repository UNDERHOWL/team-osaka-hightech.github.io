...
私のブログ テーマを使用していただきありがとうございます。簡単な操作に従って、サイト/ドキュメント/表示ページをすばやく生成します~コマンドラインは必要ありません~

[Githubでこのページを読む](https://github.com/wu-kan/jekyll-theme-WuK)
...
このリンク <https://github.com/wu-kan/wu-kan.github.io/fork> をクリックして、[サンプル サイト](https://wu-kan.cn) をあなたのウェアハウスにフォークします。フォークはサンプル サイトであり、フォーク テーマのリポジトリは不完全であることに注意してください。

<!-- .slide vertical=true -->

### 第二步

フォーク後の倉庫で、設定を選択し、倉庫名を次のように変更します`your-name.github.io`。

たとえば、ここでの例に使用した GitHub トランペットは @Mizuno-Ai で、変更された名前は`Mizuno-Ai.github.io`。
...
設定をプルダウンしてGithub Pages関連のオプションを見つけ、ソースをなしからマスターブランチに変更し、カスタムドメインを自分のURLに変更します。
...
しばらくお待ちください。ブログ アドレスにアクセスできるというメッセージが表示されます。おおむね成功～

<!-- .slide vertical=true -->

### 第四步

ウェアハウスのルートディレクトリに戻り、自分で`_config.yml`を設定して、わかる範囲で変更します。ほとんどの構成項目にコメントを追加しました。
...
yaml 形式のインデントに注意を払い、元のインデントを破壊しないことが重要です。

<!-- .slide vertical=true -->

### 第五步

ウェアハウスのルート ディレクトリに戻り、`/_posts/` フォルダをクリックします。このディレクトリにはブログの投稿が保存されています。 [新しいファイルを作成] をクリックして、独自のブログを作成します。これらの記事のフォーマットは [Markdown](https://www.runoob.com/markdown/md-tutorial.html) で、非常にシンプルで使いやすいフォーマットされたテキスト マークアップ言語です。ファイル拡張子は ` md `.
...
ファイルの命名方法は「date-title.md」である必要があります; 表示の混乱を避けるために、ファイル コンテンツの前に「title: title」の yaml ヘッダーを付けることをお勧めします。
